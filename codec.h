...
class Callback
{
 public:
  virtual ~Callback() {}
  virtual void onMessage(int socketfd,const MessagePtr& message) const = 0;
};
...
public:
  ...
  virtual void onMessage(int socketfd,const MessagePtr& message) const
  {
    std::shared_ptr<T> concrete = std::static_pointer_cast<T>(message);
    assert(concrete != NULL);
    callback_(socketfd, concrete);
  }
 private:
...
public:
    ...
    //解析数据并执行相应的回调函数
    void parse(int socketfd,muduo::net::Buffer *inputBuffer);
private:
    ...
    //根据消息类型执行相应的回调函数
    void dispatch(int socketfd,MessagePtr message);
};
