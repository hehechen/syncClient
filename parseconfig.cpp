...
ParseConfig::ParseConfig():CONFILE("syncclient.conf")
{
...
void ParseConfig::loadfile()
{
    ...
    while(fgets(linebuf,sizeof(linebuf),fp))
    {
        ...
        if(strcasecmp(key, "syncroot") == 0)//客户端同步路径
        {
            syncRoot = value;
            CHEN_LOG(DEBUG,"SYNCROOT:%s",value);
        }
        else if(strcasecmp(key, "serverip") == 0)//服务器的ip地址
        {
            serverIp = value;
            CHEN_LOG(DEBUG,"serverip:%s",value);
        }
        else
        {//用户名和密码
            username = key;
